...
const transport = ref('auto'), direction = ref('auto'), plcAddress = ref(40001)
...
async function calculate() {
  ...
  try {
    if (tool.value === 'address') {
      ...
      result.value = await invoke<string>('inspect_modbus_frame', { data: data.value, transport: transport.value, direction: direction.value, lang })
    }
...
<template>
  <ModalShell :title="t('parity.toolsTitle')" :busy="busy" wide @close="emit('close')">
    ...
    <template v-else>
      <div v-if="tool === 'frame'" class="options"><label>{{ t('dialog.transport') }}<select v-model="transport"><option value="auto">{{ t('parity.autoTransport') }}</option><option value="tcp">TCP / TLS</option><option value="rtu">RTU / RTU-over-TCP</option><option value="ascii">ASCII</option></select></label><label>{{ t('parity.frameDirection') }}<select v-model="direction"><option value="auto">{{ t('parity.autoDirection') }}</option><option value="request">{{ t('parity.request') }}</option><option value="response">{{ t('parity.response') }}</option></select></label></div>
